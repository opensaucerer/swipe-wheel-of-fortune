var SpinWheelSkins = {
  default: {
    outerCircle: {
      color: 'gradient:linear:#ececec:#d4d4d4',
      margin: 10,
      shadow: { type: 'outer' },
    },
    slice: {
    ...
    innerCircle: [
      ...
      {
        color: 'gradient:linear:#d2ff52:#7cbc0a',
        margin: 85,
        shadow: { type: 'outer' },
      },
    ],
  ...
  SpinWheel.prototype = {
    ...
    _create: function () {
      ...
      if (this.options.radius != this.radius) {
        ...
        this.scale = scale;
        this.canvas
          .attr('width', this.diameter * scale + 'px')
          .attr('height', this.diameter * scale + 'px')
          .width(this.diameter * scale)
          .height(this.diameter * scale);
    ...
    _parseColor: function (color, center, radius, is_circle = false) {
      ...
      if (typeof color == 'object') {
        if (color.type == 'radial') {
          if (color.positions) {
            var grd = this.ctx.createRadialGradient.apply(
              this.ctx,
              color.positions
            );
          } else {
          ...
          if (color.positions) {
            var grd = this.ctx.createLinearGradient.apply(
              this.ctx,
              color.positions
            );
          } else {
    ...
    _drawCircle: function (props) {
      ...
      if (props.shadow && props.shadow.type == 'outer') {
        ...
        this.ctx.shadowBlur =
          (props.shadow.blur ? props.shadow.blur : 5) * this.scale;
        this.ctx.shadowColor = props.shadow.color
          ? props.shadow.color
          : 'rgba(0, 0, 0, 0.6)';
        this.ctx.shadowOffsetX =
          (props.shadow.offsetX ? props.shadow.offsetX : 0) * this.scale;
        this.ctx.shadowOffsetY =
          (props.shadow.offsetY ? props.shadow.offsetY : 0) * this.scale;
      }

      if (props.color) {
        this.ctx.fillStyle = this._parseColor(
          props.color,
          center + offsetX,
          radius,
          angle == this._toRadian(360)
        );
        this.ctx.fill();
      }
      ...
      if (props.image) {
        ...
        if (!props.patternImage) {
          ...
          //this.ctx.clip();
          var r = radius * 0.9;
          var pattern = this.ctx.drawImage(
            props.patternImage,
            -r,
            -r,
            2 * r,
    ...
    _wrapText: function (text, props, maxWidth, lineHeight, y = 0) {
      ...
      for (var n = 0; n < words.length; n++) {
        ...
        if (testWidth > maxWidth && n > 0) {
          if (props.stroke)
            this.ctx.strokeText(line, -this.ctx.measureText(line).width / 2, y);
    ...
    _resizeText: function (text, sw, props) {
      ...
      if (props.stroke)
        this.ctx.strokeText(text, -this.ctx.measureText(text).width / 2, 0);
    ...
    _drawText: function (start_angle, angle, props) {
      ...
      var mul =
        this.options.slices.length <= ms && !props.show_vertical
          ? props.radius
          : this.availarea / 2 + this.innerarea;
      var offsetX = Math.cos(mid_angle) * mul;
      var offsetY = Math.sin(mid_angle) * mul;
      if (props.shadow) {
        this.ctx.shadowOffsetX = props.shadow.offsetX
          ? props.shadow.offsetX
          : 0;
        this.ctx.shadowOffsetY = props.shadow.offsetY
          ? props.shadow.offsetY
          : 0;
        this.ctx.shadowBlur =
          (props.shadow.blur ? props.shadow.blur : 0) * this.scale;
        this.ctx.shadowColor = props.shadow.color;
      }
      ...
      if (this.options.slices.length <= ms && !props.show_vertical) {
        var a =
          Math.cos(start_angle) * props.radius -
          Math.cos(start_angle + angle) * props.radius;
        var b =
          Math.sin(start_angle) * props.radius -
          Math.sin(start_angle + angle) * props.radius;
    ...
    _drawSlices: function (props) {
      ...
      for (var i = 0; i < slices.length; i++) {
        ...
        if (props.bgcolors) {
          params['color'] =
            slice.style && slice.style.color
              ? slice.style.color
              : props.bgcolors[clridx];
          clridx++;
        ...
        $.extend(params.text, {
          text: text,
          radius: this.radius * 0.8 - props.margin,
        });
        //console.log(params);
        ...
        if (
          slicea >= this._toRadian(this.arrow_angle) &&
          slicea <= this._toRadian(this.arrow_angle + 10) &&
          this.rotate_arrow == 0 &&
          this.rotating
        ) {
          this._onTick();
